...
def main():
    ...
    ap = argparse.ArgumentParser(description="Run color experiment inference + metrics (resumable)")
    ...
    for model_name in model_names:
        ...
        result_dir = proj_root / "results" / "color" / model_slug / dataset_name / run_id
        (result_dir / "raw_outputs").mkdir(parents=True, exist_ok=True)
        (result_dir / "logs").mkdir(parents=True, exist_ok=True)
        (result_dir / "analysis").mkdir(parents=True, exist_ok=True)
        ...
        # Calculate total operations (images * prompt_types)
        base_total = sum(len(exp['images']) for exp in manifest['experiments'])
        total = base_total * len(prompt_types)
        ...
        # Load resume progress if requested
        progress_path = result_dir / "raw_outputs" / "progress.json"
        completed: Set[str] = set()
        do_checkpoint = bool(args.enable_checkpoint and args.checkpoint_interval > 0)
        if args.resume and do_checkpoint and progress_path.exists():
            try:
                with open(progress_path.as_posix(), 'r', encoding='utf-8') as pf:
                    data = json.load(pf)
        ...
        def flush_progress():
            if not do_checkpoint:
                return
            try:
                with open(progress_path.as_posix(), 'w', encoding='utf-8') as pf:
                    json.dump({
                        'timestamp': datetime.now().isoformat(),
                        'model_name': model_name,
                        'dataset_name': dataset_name,
                        'task_name': task_name,
                        'completed_keys': sorted(list(completed)),
                    }, pf, ensure_ascii=False, indent=2)
            except Exception:
                pass

        pbar = tqdm(total=total, desc=f"Color Inference ({model_slug})")
        ...
        for exp in outputs['experiments']:
            # Initialize VLM outputs structure for multiple prompt types
            vlm_out = {pt: {} for pt in prompt_types}
                ...
                try:
                    with open(exp_file.as_posix(), 'r', encoding='utf-8') as ef:
                        ...
                        for pt in prompt_types:
                            vlm_out[pt].update(prev.get('vlm_outputs', {}).get(pt, {}))
                except Exception:
                    pass
            
            for cond, image_path in exp['images'].items():
                # Run inference for each prompt type
                for prompt_type in prompt_types:
                    key = f"{text_id}/{cond}/{prompt_type}"
                    if args.resume and key in completed and cond in vlm_out[prompt_type]:
                        pbar.update(1)
                        continue
                    ...
                    try:
                        ...
                        if task_name == 'qa' and 'qa' in exp:
                            ...
                            prompt = SENTIMENT_PROMPT_FORCED_CHOICE if prompt_type == "forced_choice" else SENTIMENT_PROMPT
                        text = handler.generate(img, prompt, temperature=args.temperature, num_beams=args.num_beams)
                    except Exception as e:
                        text = f"[ERROR] {e}"
                    vlm_out[prompt_type][cond] = text
                    # Sentiment analysis for this prompt type and condition
                    if task_name == 'sentiment':
                        text = vlm_out[prompt_type][cond]
                        if prompt_type == "forced_choice":
                            ...
                            try:
                                ...
                                per_condition_sentiments[f"{cond}_{prompt_type}"].append(signed)
                            except Exception as e:
        ...
        if task_name == 'qa':
            ...
            if decoy_induced_errors:
                ...
                print(f"💾 Saved induced error metrics: {induced_csv_path}")
            
            # 通常のQA metrics CSV
            csv_path = (result_dir / "analysis" / "qa_metrics.csv").as_posix()
        ...
        # Sentiment metrics per condition with bias vs baseline (for both prompt types)
            ...
            for prompt_type in prompt_types:
                # Filter conditions for this prompt type
                prompt_conditions = {k: v for k, v in per_condition_sentiments.items() if k.endswith(f"_{prompt_type}")}
                
                if prompt_conditions:
                    baseline_key = f'baseline_black_{prompt_type}'
                    base_vals = prompt_conditions.get(baseline_key, [])
                    baseline_avg = (sum(base_vals)/len(base_vals)) if base_vals else None
                    
                    for cond, vals in prompt_conditions.items():
                    ...
                    # Create separate CSV for each prompt type
                    csv_path = (result_dir / "analysis" / f"sentiment_metrics_{prompt_type}.csv").as_posix()
                    with open(csv_path, 'w', newline='', encoding='utf-8') as cf:
                        writer = csv.writer(cf)
                        writer.writerow(["condition", "signed_score_avg", "bias_vs_baseline", "n"])
                        for cond, m in outputs['sentiment_metrics_per_condition'].items():
                            if cond.endswith(f"_{prompt_type}"):
                                # Remove prompt type suffix for cleaner condition names in CSV
                                clean_cond = cond.replace(f"_{prompt_type}", "")
                                bias_val = "" if m['bias_vs_baseline'] is None else f"{m['bias_vs_baseline']:.6f}"
                                writer.writerow([clean_cond, f"{m['signed_score_avg']:.6f}", bias_val, m['n']])
                    print(f"💾 Saved sentiment metrics ({prompt_type}): {csv_path}")
            
            # Also create a combined CSV for comparison
            combined_csv_path = (result_dir / "analysis" / "sentiment_metrics_combined.csv").as_posix()
            with open(combined_csv_path, 'w', newline='', encoding='utf-8') as cf:
                writer = csv.writer(cf)
                writer.writerow(["condition", "prompt_type", "signed_score_avg", "bias_vs_baseline", "n"])
                for cond, m in outputs['sentiment_metrics_per_condition'].items():
                    for prompt_type in prompt_types:
                        if cond.endswith(f"_{prompt_type}"):
                            clean_cond = cond.replace(f"_{prompt_type}", "")
                            bias_val = "" if m['bias_vs_baseline'] is None else f"{m['bias_vs_baseline']:.6f}"
                            writer.writerow([clean_cond, prompt_type, f"{m['signed_score_avg']:.6f}", bias_val, m['n']])
            print(f"💾 Saved combined sentiment metrics: {combined_csv_path}")
        ...
        try:
            ...
            # Also emit a compact status CSV to quickly see coverage
